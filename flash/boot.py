...
#------------------------------------------------------
try:
    SYSLOG_SERVER_IP = ConfigData['SYSLOG_SERVER_IP']
    print ('Syslog Server = {}'.format(SYSLOG_SERVER_IP))
except KeyError:
    print("Syslog Server IP not configured revert to 192.168.1.150")

GardenaSyslog = usyslog.UDPClient(ip=ConfigData['SYSLOG_SERVER_IP'])
GardenaSyslog.info('GardenaValveControl:Booting Version 2.0')